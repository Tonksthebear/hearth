...
class DuplicatedMigrationVersionRepairTest < ActiveSupport::TestCase
  ...
  STARTING_AGENT_RUNTIME_STATUS_VERSION = 20260802020000
  PANTRY_ITEMS_VERSION = 20260805010000
  PLANNED_MEAL_INGREDIENTS_VERSION = 20260805020000
  PLANNED_MEAL_ALLOCATION_PRIORITY_VERSION = 20260805030000
  SHOPPING_DEFICIT_PROVENANCE_VERSION = 20260805040000
  PANTRY_CONSUMPTIONS_VERSION = 20260805050000
  EXERCISE_VISUALS_VERSION = 20260825140000
  EXERCISE_VISUAL_ITEMS_VERSION = 20260825140100
  MUSCLE_TAXONOMY_VERSION = 20260825150200
  SOURCE_TRACKING_VERSION = 20260825210000
  IMPORT_RUNS_VERSION = 20260826120000
  ...
  test "migration versions have one owner" do
    ...
    [ RUNTIME_VERSION, WORKOUT_VERSION, RECIPE_VERSION, RECONCILIATION_VERSION, MEAL_EVENTS_VERSION, SHOPPING_VERSION, NUTRITION_VERSION, GUARDED_MUTATIONS_VERSION, LORESTER_KNOWLEDGE_VERSION, AGENT_CHAT_VERSION, EXPLICIT_AGENT_AUTHENTICATION_VERSION, STABLE_AGENT_INSTALLATION_IDENTITY_VERSION, WEB_AGENT_SETUP_VERSION, STARTING_AGENT_RUNTIME_STATUS_VERSION, PANTRY_ITEMS_VERSION, PLANNED_MEAL_INGREDIENTS_VERSION, PLANNED_MEAL_ALLOCATION_PRIORITY_VERSION, SHOPPING_DEFICIT_PROVENANCE_VERSION, PANTRY_CONSUMPTIONS_VERSION, EXERCISE_VISUALS_VERSION, EXERCISE_VISUAL_ITEMS_VERSION, MUSCLE_TAXONOMY_VERSION, SOURCE_TRACKING_VERSION, IMPORT_RUNS_VERSION ].each do |version|
  ...
  test "fresh and collided histories converge without losing data" do
    canonical_schema = with_isolated_database do |connection, context, pool|
      ...
      assert_equal [ RUNTIME_VERSION, WORKOUT_VERSION, RECIPE_VERSION, RECONCILIATION_VERSION, MEAL_EVENTS_VERSION, SHOPPING_VERSION, NUTRITION_VERSION, GUARDED_MUTATIONS_VERSION, LORESTER_KNOWLEDGE_VERSION, AGENT_CHAT_VERSION, AGENT_TURN_DIAGNOSTICS_VERSION, EXPLICIT_AGENT_AUTHENTICATION_VERSION, STABLE_AGENT_INSTALLATION_IDENTITY_VERSION, WEB_AGENT_SETUP_VERSION, STARTING_AGENT_RUNTIME_STATUS_VERSION, PANTRY_ITEMS_VERSION, PLANNED_MEAL_INGREDIENTS_VERSION, PLANNED_MEAL_ALLOCATION_PRIORITY_VERSION, SHOPPING_DEFICIT_PROVENANCE_VERSION, PANTRY_CONSUMPTIONS_VERSION, EXERCISE_VISUALS_VERSION, EXERCISE_VISUAL_ITEMS_VERSION, MUSCLE_TAXONOMY_VERSION, SOURCE_TRACKING_VERSION, IMPORT_RUNS_VERSION ],
  ...
  private
    ...
    def assert_supported_final_state(connection)
      assert connection.table_exists?(:exercise_visuals)
      assert connection.table_exists?(:exercise_visual_items)
      assert connection.table_exists?(:muscles)
      assert connection.table_exists?(:exercise_muscle_targets)
      assert connection.column_exists?(:exercises, :source_key)
      assert connection.column_exists?(:exercises, :source_version)
      assert connection.column_exists?(:exercises, :source_snapshot)
      assert connection.column_exists?(:exercises, :source_removed_at)
      assert connection.table_exists?(:workout_guide_import_runs)
      expected_muscles = Muscle::DEFAULTS.map { |row| [ row.fetch(:key), row.fetch(:name), row.fetch(:muscle_group), row.fetch(:display_position).to_s ] }
      actual_muscles = connection.select_rows(<<~SQL.squish).map { |row| [ row[0], row[1], row[2], row[3].to_s ] }
        SELECT key, name, muscle_group, display_position
        FROM muscles
        ORDER BY display_position
      SQL
      assert_equal expected_muscles, actual_muscles
