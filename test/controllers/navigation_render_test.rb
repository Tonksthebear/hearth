...
class NavigationRenderTest < ActionDispatch::IntegrationTest
  ...
  test "shared shell exposes the four primary destinations and relocates administration" do
    ...
    assert_select "nav[aria-label='Primary']", count: 2 do |navigations|
      navigations.each do |navigation|
        assert_select navigation, "> ul > li > a", count: 4
      end
    end
    assert_select "nav[aria-label='Primary'] a", text: "Today"
    assert_select "nav[aria-label='Primary'] a", text: "Meals"
    assert_select "nav[aria-label='Primary'] a", text: "Activities"
    assert_select "nav[aria-label='Primary'] a", text: "Coach"
    %w[Manage\ people Recipes Shopping Exercises Workout\ templates Recovery].each do |label|
      assert_select "nav[aria-label='Primary'] a", text: label, count: 0
    end
    assert_select "button[data-account-menu-trigger]", text: /#{Regexp.escape(households(:home).name)}.*Settings and account/m, count: 2
    assert_select "[data-account-menu]", text: /Manage people/
    assert_select "[data-account-menu]", text: /Manage recipes/
    assert_select "[data-account-menu]", text: /Workout templates/
    assert_select "[data-account-menu]", text: /Exercises/
    assert_select "[data-account-menu]", text: /Habits/
    assert_select "form", text: /Switch to/, count: 0
  end
  ...
  test "administrative pages do not falsely mark a primary destination current" do
    ...
    assert_response :success
    assert_select "nav[aria-label='Primary'] a[aria-current='page']", count: 0
  end
