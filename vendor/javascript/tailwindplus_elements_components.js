...
function removeStaleSlimSelectMarkup(select) {
  const nextElement = select.nextElementSibling
  if (nextElement?.classList.contains("ss-main")) nextElement.remove()

  const slimSelectId = select.dataset.id || select.dataset.ssid
  if (!slimSelectId) return

  document.querySelectorAll(`.ss-content[data-id="${escapeCssIdentifier(slimSelectId)}"]`).forEach((content) => {
    content.remove()
  })
}

function findSlimSelectElements(root = document) {
  const elements = []

  if (root.matches?.("select[data-elements-autocomplete]")) {
    elements.push(root)
  }

  root.querySelectorAll?.("select[data-elements-autocomplete]").forEach((select) => {
...
function parseSlimSelectOptions(select) {
  if (!select.dataset.slimSelectOptions) return {}

  try {
    return JSON.parse(select.dataset.slimSelectOptions)
  } catch (error) {
    console.warn("Invalid SlimSelect options JSON", { select, error })
    return {}
  }
}

function resolveSlimSelectOptions(select, options) {
  const settings = { ...(options.settings || {}) }

  if (!Object.prototype.hasOwnProperty.call(settings, "contentLocation")) {
    const dialogContainer = select.closest("el-dialog-panel") || select.closest("dialog")
    if (dialogContainer) {
      settings.contentLocation = dialogContainer
      if (!Object.prototype.hasOwnProperty.call(settings, "contentPosition")) {
        settings.contentPosition = "fixed"
      }
    }
  }
...
function registeredSlimSelectEvents(select) {
  const name = select.dataset.slimSelectEvents
  if (!name) return {}

  return window.TailwindPlusElements.slimSelectEvents[name] || {}
}

function selectHasOption(select, value) {
  const normalizedValue = value.toLowerCase()
  return Array.from(select.options).some((option) => {
    return option.value.toLowerCase() === normalizedValue || option.text.trim().toLowerCase() === normalizedValue
  })
}
...
function syncSlimSelectAria(select, instance) {
  const main = instance.render?.main?.main || select.nextElementSibling
  if (!main) return
  ...
  if (describedBy) main.setAttribute("aria-describedby", describedBy)
}

function destroySlimSelectsForStream(stream) {
  const target = stream.getAttribute("target")
  const targets = stream.getAttribute("targets")

  if (target) {
    const element = document.getElementById(target)
    if (element) destroySlimSelects(element)
  }

  if (targets) {
    document.querySelectorAll(targets).forEach((element) => destroySlimSelects(element))
  }
}

function resetDialogsBeforeTurboCache() {
  document.querySelectorAll("el-dialog").forEach((dialogElement) => {
    dialogElement.removeAttribute("open")
    clearTransitionState(dialogElement)

    dialogElement.querySelectorAll("dialog[open]").forEach((dialog) => {
      try {
        dialog.close()
      } catch (_) {
    ...
    dialogElement.querySelectorAll("[data-closed], [data-enter], [data-leave], [data-transition]").forEach(clearTransitionState)
  })

  resetDocumentScrollLock()
}

function resetPopoversBeforeTurboCache() {
...
function resetDocumentScrollLock() {
  ...
  root.style.removeProperty("--el-top-layer-scrollbar-offset")
  root.style.removeProperty("overflow")
  root.style.removeProperty("padding-right")
}

document.addEventListener("turbo:before-cache", () => {
  prepareSlimSelectsForCache()
  resetDialogsBeforeTurboCache()
  resetPopoversBeforeTurboCache()
})

document.addEventListener("turbo:before-frame-render", (event) => destroySlimSelects(event.target))
document.addEventListener("turbo:before-stream-render", (event) => {
  destroySlimSelectsForStream(event.target)
...
document.addEventListener("turbo:before-visit", () => {
  document.documentElement.removeAttribute("data-elements-ready")
})
async function markElementsReady() {
  await Promise.all([
    customElements.whenDefined("el-dialog"),
