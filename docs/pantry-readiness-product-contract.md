...
Purchase confirmation and shopping deficits remain scaffold-only. Today `ShoppingList#reconcile!` is reached by `ShoppingListsController#show` and `PlannedMeal` after-commit callbacks, while Meals, Today, and MCP projections consume an existing list without reconciling it. The follow-up cold-switch ticket must consume `Household::PantryAllocation` deficits through those production seams and prove the actual user/runtime path. Until that ticket lands, a past plan that was never cooked keeps its place in the queue and holds its reservation.
