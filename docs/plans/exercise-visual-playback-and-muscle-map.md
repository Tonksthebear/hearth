...
### Response to Plan Review `review_1787768767_870238` (revision 4)

The reviewer confirmed that revision 3 closed the accessibility, full-item identity, GIF, and
timer-alias findings. Two findings remain, both correct.
...
| The dark surface applies to unchanged source art **and only to it** | **Both arms are proven in one rendered test**, because a model-tier negative alone would let an implementation that applies the dark surface unconditionally pass every check. Positive arm: an imported Workout Guide exercise whose frames are untouched renders under emulated `prefers-color-scheme: light` with a dark computed background on the frame container. Negative arm: the same exercise after a household replaces one frame file renders under the same emulated light mode with the default computed background, not the dark one. The test asserts the two computed values differ, so neither arm can pass by both being the same colour |
...
- The dark surface has a rendered negative arm, not only a model-tier one (R11).
